...
The model combines five macro indicators into a single composite score, normalized using a rolling Z-score to eliminate look-ahead bias. The composite is mapped to one of four regimes (**Expansion**, **Late Cycle**, **Slowdown**, **Contraction**) and paired with a momentum signal to capture directional drift within a regime.
...
### Normalization
Each indicator is normalized using a **rolling 60-month Z-score** (minimum 36 months). Full-sample normalization uses future data to scale past observations, which introduces look-ahead bias and makes backtests misleading. The rolling window ensures each month's score reflects only information that was available at that point in time.

### Composite Score
Indicators are combined using **equal weights (1/N)**. Per DeMiguel, Garlappi & Uppal (2009), naive diversification consistently outperforms statistically optimized weights out-of-sample due to estimation error in the covariance matrix. Optimization fits historical noise rather than signal.
...
Standard deviation understates tail risk in financial returns, which are fat-tailed. VaR addresses this partially but only identifies the loss threshold; it says nothing about the magnitude of losses beyond that point. CVaR is the mean return in the worst 5% of months, which makes it a more complete tail risk measure and the standard in institutional risk management.
...
| 10Y Treasury | FRED GS10 + duration approximation | No free total return index available; return approximated as -8 x Δyield |
...
Notable: bonds returned -0.23% on average during Slowdown periods (dominated by the 2022 rate shock), which illustrates the breakdown of the traditional 60/40 hedge. Gold acted as a Contraction hedge (+1.27% avg) but provided minimal protection in Slowdown (+0.25%).
...
- [FRED (Federal Reserve Economic Data)](https://fred.stlouisfed.org/) via OpenBB
- [yfinance](https://github.com/ranaroussi/yfinance) for S&P 500 and GLD price data
- [NBER recession dating](https://www.nber.org/research/business-cycle-dating) for recession shading (USREC via FRED)
